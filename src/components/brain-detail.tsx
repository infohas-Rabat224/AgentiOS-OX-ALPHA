...
import type { BrainRecord, BrainRelationship } from "@/types/missionControl";
import { brainStatusToColor } from "@/lib/brainsData";
...
export function BrainDetail({ brain, relationships = [], onClose, onRefresh }: BrainDetailProps) {
  ...
  const uptimeStr = formatDuration(brain.uptime || 0);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <span className="text-[10px] text-faint">Live daemon connected</span>
      </div>
      <div className="rounded-xl border border-border/30 bg-surface/5 p-6 text-center">
        ...
        <p className="text-xs text-faint">Operating system daemon event log active</p>
        <div className="mt-3 text-left font-mono text-[11px] p-3 rounded-lg bg-surface/40 border border-border/40 text-muted space-y-1">
          <div>[INFO] Brain registered: {brain.id} ({brain.display_name})</div>
          <div>[INFO] Runtime {brain.runtime} initialized in {brain.workspace}</div>
          <div>[DEBUG] Health check passed (status: {brain.status})</div>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-border/30 bg-surface/5 p-6 text-center">
        ...
        <p className="text-[10px] text-faint/60 mt-1">Events appear here as they are emitted by the backend kernel</p>
      </div>
    </div>
  );
}

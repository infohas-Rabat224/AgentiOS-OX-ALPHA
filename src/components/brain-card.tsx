...
import type { BrainRecord, BrainVendor } from "@/types/missionControl";
import { brainStatusToColor, VENDOR_ICON_MAP } from "@/lib/brainsData";
import { type ReactNode } from "react";

// ── Vendor icon ─────────────────────────────────────────────────────────────

export function VendorIcon({ vendor, size = 16 }: { vendor: BrainVendor; size?: number }) {
  const color = (VENDOR_ICON_MAP as any)[vendor] ?? "#94a3b8";
  const initial = (vendor || "c").charAt(0).toUpperCase();

  return (
    <span
      className="inline-flex items-center justify-center rounded-full font-bold text-white shadow-sm"
      style={{
